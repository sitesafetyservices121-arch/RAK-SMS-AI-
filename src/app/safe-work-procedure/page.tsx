...
export default function SafeWorkProcedurePage() {
  ...
  async function onSubmit(values: FormValues) {
    ...
    if (response.success && response.data) {
      ...
      toast({
        title: "Success",
        description: "Safe Work Procedure generated successfully. You can now preview or download it.",
      });
    } else {
      toast({
        variant: "destructive",
  ...
  const generatePdf = (data: GenerateSafeWorkProcedureOutput, values: FormValues, logo: string | null, setUri = false) => {
    ...
    doc.setFontSize(16);
    doc.text(`Task: ${values.taskDescription.substring(0, 50)}...`, 105, 150, { align: "center" });
    ...
    const sections = data.procedure.split('## ').slice(1);
    ...
    sections.forEach(section => {
        ...
        splitContent.forEach((line: string) => {
            ...
            doc.text(line.replace(/•/g, '  •'), 14, y); // Indent list items
            y += 6;
        });
        y += 10;
    });
  ...
  const handleSave = () => {
    toast({
        ...
        description: "The Safe Work Procedure has been saved to the 'Generated Documents' library."
    });
  }
                  ...
                  <Dialog>
                    ...
                    <DialogContent className="max-w-4xl h-[90vh]">
                        ...
                        <div className="h-full">
                           {pdfDataUri && <iframe src={pdfDataUri} width="100%" height="100%" title="Safe Work Procedure Preview" />}
                        </div>
                        <DialogFooter>
