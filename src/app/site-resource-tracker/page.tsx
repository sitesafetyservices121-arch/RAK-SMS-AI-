...
export default function SiteResourceTrackerPage() {
  const [employeeAssignments, setEmployeeAssignments] = useState<Record<string, string>>({}); // employeeId -> siteId
  const [vehicleAssignments, setVehicleAssignments] = useState<Record<string, string>>({}); // vehicleName -> siteId
  ...
  const handleSaveToGenerated = () => {
    // In a real app, this would trigger a server action to save the PDF to cloud storage
    // and create a record in the 'Generated Documents' database.
    toast({
        title: "Report Saved",
                    ...
                    <Card key={site.id}>
                        ...
                        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                {initialEmployees.filter(emp => employeeAssignments[emp.id] === site.id).map(emp => (
                                    <div key={emp.id} className="text-sm text-muted-foreground p-2 border rounded-md">
                                    {emp.firstName} {emp.surname}
                                    </div>
                                ))}
                                {initialEmployees.filter(emp => employeeAssignments[emp.id] === site.id).length === 0 && (
                            ...
                            {/* Assigned Vehicles */}
                             <div>
                                ...
                                <div className="space-y-2">
                                {availableVehicles.filter(v => vehicleAssignments[v.vehicle] === site.id).map(v => (
                                    <div key={v.vehicle} className="text-sm text-muted-foreground p-2 border rounded-md">
                                    {v.vehicle}
                                    </div>
                                ))}
                                {availableVehicles.filter(v => vehicleAssignments[v.vehicle] === site.id).length === 0 && (
                        ...
                        <CardFooter className="flex flex-wrap gap-2 pt-4">
                            <p className="text-xs font-semibold w-full text-muted-foreground">Assign Resources:</p>
                             {initialEmployees.filter(emp => employeeAssignments[emp.id] !== site.id).map(emp => (
                                <Button key={`assign-emp-${emp.id}`} size="sm" variant="outline" onClick={() => handleAssignEmployee(emp.id, site.id)}>
                                    <User className="mr-2 h-4 w-4" /> {emp.firstName}
                                </Button>
                            ))}
                            {availableVehicles.filter(v => vehicleAssignments[v.vehicle] !== site.id).map(v => (
                                <Button key={`assign-veh-${v.vehicle}`} size="sm" variant="outline" onClick={() => handleAssignVehicle(v.vehicle, site.id)}>
