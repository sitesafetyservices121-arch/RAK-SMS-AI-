...
export default function PpeIssueRegisterPage() {
  ...
  const employeeMap = useMemo(
    () =>
      new Map(initialEmployees.map((e) => [e.id, `${e.firstName} ${e.surname}`])),
    []
  );
  ...
  const groupedByEmployee = useMemo(() => {
    ...
    // Sort each employee's history by newest first
    map.forEach((entries) =>
      entries.sort(
  ...
  const handleIssuePpe = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const validityMonths = parseInt(formData.get("validity") as string, 10);
