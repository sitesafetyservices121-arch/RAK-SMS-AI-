...
export async function generatePayFastSignatureAction(paymentData: PayFastPaymentData): Promise<{ success: boolean; data?: PayFastSignatureData; error?: string }> {
    try {
        ...
        if (!merchantId || !merchantKey || !passphrase) {
            throw new Error("PayFast credentials are not configured in the environment variables.");
        }
        ...
        const signatureString = `${queryString}&passphrase=${encodeURIComponent(passphrase.trim()).replace(/%20/g, '+')}`;
